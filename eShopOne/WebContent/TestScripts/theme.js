...
var pandora = {
	...
	initNavigationSelector: function() {
		...
		if(selector.length > 0)
		{
			...
			$(".navigationSelector").change(function(e){
	...
	navigationSelectorHeader: function() {
		var selector = $('.navigationSelectorHeader');
		if(selector.length > 0)
		{
			selector.change(function(){
				window.location = $('option:selected', $(this)).attr('value');
	...
	initCart: function() {
		...
		// + 1 quantity
		$('.main-cart .quantity .more').on('click', function(){
		...
		// - 1 quantity
		$('.main-cart .quantity .less').on('click', function(){
			var quantity = $(this).siblings('input[type=text]');
		...
		$('.main-cart .quantity input[type=text]').on('keydown', function(event) {
	...
	adjustQuickShopPopupPosition: function(product) {
		$('.quick-shop').css({
			position: 'fixed',
	...
	adjustQuickShopPopupPosition_view: function(product) {
		$('.quick-shop_view').css({
			position: 'fixed',
			width: '960px',
  			padding: '10px',
	...
	closeQuickShopPopup_view: function() {
		
		$('.quick-shop_view').fadeOut(350);			//close popup
		$('.lightbox').hide();
	},
	initProductLightbox: function() {
		...
		$('.lightbox-launcher').click(function(){
			...
			productImage.onload = function() {
				...
				$('.quick-shop_view').html(productImage);
				$('.quick-shop_view').append('<a href="#" class="close"></a>');
				pandora.adjustQuickShopPopupPosition_view();
				$('.quick-shop_view').fadeIn(350, function(){
					pandora.adjustQuickShopPopupPosition_view();
				});
			};
		...
		$('.lightbox, .close, .quick-shop_view > img').live('click', function(){
			pandora.closeQuickShopPopup_view();
			return false;
		});
